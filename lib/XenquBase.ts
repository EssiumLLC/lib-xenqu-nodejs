...
export default class XenquBase {
    ...
    private errorCallbacks: {
        [key: string]: ((data?: XenquApiError) => void) | undefined;
    };
    private oauth2RetryVars: {
        clientId: string;
        clientSecret: string;
        subscriber: string;
        privateKey: string;
    ...
    private makeGetRecursive(
        ...
        return fetch(this.baseUrl + path + params, {
            method: "GET",
            headers: {
                authorization: this.getOAuth1Headers("GET", path, parameters),
            },
        })
            .then((res: Response) => {
                if (res.ok) {
                    ...
                    // if we get 401, automatically retry auth
                    return this.makeOAuth2Request(
                        this.oauth2RetryVars.clientId,
                        this.oauth2RetryVars.clientSecret,
                        this.oauth2RetryVars.subscriber,
                        this.oauth2RetryVars.privateKey
                    ).then(() => {
                        return this.makeGetRecursive(
                            path,
                            parameters,
                            retries + 1
                        );
    ...
    private makePostRecursive(
        ...
        return fetch(this.baseUrl + path + params, {
            method: "POST",
            headers: {
                authorization: this.getOAuth1Headers("POST", path, parameters),
    ...
    private makePutRecursive(
        ...
        return fetch(this.baseUrl + path + params, {
            method: "PUT",
            headers: {
                authorization: this.getOAuth1Headers("PUT", path, parameters),
                "Content-Type": "application/json",
            },
            body: payload,
        })
    ...
    private makeDeleteRecursive(
        ...
        return fetch(this.baseUrl + path + params, {
            method: "DELETE",
            headers: {
                authorization: this.getOAuth1Headers(
                    "DELETE",
                    path,
                    parameters
                ),
            ...
            .then((res: Response) => {
                if (res.ok) {
                    ...
                    return this.makeOAuth2Request(
                        ...
                        return this.makeDeleteRecursive(
                            path,
                            payload,
                            parameters,
                            retries + 1
     ...
     */
    async makeOAuth2Request(
        clientId: string,
        clientSecret: string,
        subscriber: string,
        privateKey: string
    ): Promise<boolean> {
        this.oauth2RetryVars = {
            clientId: clientId,
        ...
        const aud =
            this.baseUrl.replace(/\/api\/?$/, "") || "https://xenqu.com";
        const payload = {
    ...
    requestToken(callback: string): Promise<any> {
        ...
        const oauth = this.getOauth1HeadersWebAuth(
            "POST",
            url,
            false,
            callback
    ...
    refreshToken(): Promise<boolean> {
        ...
        const oauth = this.useWebAuth
            ? this.getOauth1HeadersWebAuth("POST", url, true)
            : this.getOAuth1Headers("POST", url);
    ...
    authenticate(
        ...
        const toSign = {
            ...additionalParameters,
            temp_token: this.webOauth.token,
        ...
        let formData = "";
        for (const key in toSign)
            if (toSign.hasOwnProperty(key))
                formData += `&${key}=${toSign[key]}`;
    ...
    getOAuth1Credentials(): OAuth1Credentials {
        return {
            ...
            token_secret: !this.useWebAuth
                ? this.oauth.secret
                : this.webOauth.secret,
        };
    }
     ...
     * @private
     */
    private getOauth1HeadersWebAuth(
        httpMethod: string,
        url: string,
        signPersonally: boolean,
        callback?: string,
        additionalParams?: { [key: string]: any },
        clientId?: string,
        clientSecret?: string
        ...
        const keys = {
            ...
            verifier:
                signPersonally && this.webOauth.verifier !== ""
                    ? this.webOauth.verifier
                    : undefined,
        };
    ...
    registerErrorHandler(
        id: string,
        callback: (data?: XenquApiError) => void
    ): { id: string } {
    ...
    private executeAllErrorHandlers(error: XenquApiError) {
        Object.keys(this.errorCallbacks).forEach((key: string) => {
            try {
                if (
                    this.errorCallbacks[key] &&
                    typeof this.errorCallbacks[key] === "function"
                ) {
