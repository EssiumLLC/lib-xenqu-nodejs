...
export default class XenquAPI {
    ...
    constructor(
        ...
        privateKey?: string,
        subscriber?: string,
        url: string = "https://xenqu.com/api",
        noPK: boolean = false
    ) {
    ...
    public tryOldCredentials(
        ...
        if (!this.isInit) {
            ...
            if (autoRenew) {
                ...
                return new Promise<OAuth1Credentials>((resolve) => {
                    resolve(XenquAPI.Base.getOAuth1Credentials());
                });
            }
        } else {
            throw new Error("Already initiated!");
    ...
    /**
     * Set the subscriber (JWT sub) used for OAuth2 impersonation.
     */
    public setSubscriber(subscriber: string): void {
        this.subscriber = subscriber;
        this.isInit = false;
    }

    public getSubscriber(): string {
    ...
    public reauth(): Promise<OAuth1Credentials> {
        this.isInit = false;
        if (!this.useWebFlowAuthentication) {
            return XenquAPI.Base.makeOAuth2Request(
                this.clientId,
                this.clientSecret,
                this.subscriber,
                this.privateKey
    ...
    public authenticate(
        ...
        additionalParameters: AdditionalLoginParameters
    ): Promise<string> {
        if (this.useWebFlowAuthentication) {
            return XenquAPI.Base.authenticate(
                clientId,
                clientSecret,
                callback,
                authenticator,
            ...
            throw new Error(
                "Xenqu-Api was not initialized to use this authentication method! Please use init()."
            );
        }
    }
     ...
     * @param additionalParameters Additional parameters needed for the signin
     */
    public attemptAuthWithUNandPWorSSO(
        clientId: string,
        clientSecret: string,
        callback: string,
        authCallback: string,
        ...
        this.isInit = false;
        if (this.useWebFlowAuthentication) {
            return XenquAPI.Base.authenticateWithSSOorUNandPW(
                clientId,
                clientSecret,
                callback,
                authCallback,
