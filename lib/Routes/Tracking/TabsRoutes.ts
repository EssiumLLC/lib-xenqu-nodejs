...
export default class TabsRoutes {
    ...
    getTemplates(groupId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/queue_templates`
        );
    }
    ...
    getTemplate(groupId: string, templateId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/queue_templates/${templateId}`
        );
    }
    ...
    /**
     * Get a single library on a tab.
     */
    getLibrary(groupId: string, libraryId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/libraries/${libraryId}`
        );
    }

    /**
     * Get tab detail with optional includes (datasources, automation_settings, form_defaults, follow_setup).
     */
    getTab(groupId: string, include?: string[]) {
        const params: Record<string, string> = {};
        if (include && include.length > 0) {
            params.include = include.join(",");
        }
        return XenquAPI.Base.makeGet(`/tracking/groups/${groupId}`, params);
    }

    /**
     * Get a tab member including per-user tab_settings.
     */
    getTabMember(groupId: string, memberId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/members/${memberId}`
        );
    }

    /**
     * Get a tab list item definition by item id.
     */
    getTabItem(groupId: string, itemId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/list/items/${itemId}`
        );
    }

    /**
     * List automation rules on a tab.
     */
    listAutomationRules(groupId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/automation_rules`
        );
    }
     ...
     */
    getAutomationRule(groupId: string, ruleId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/automation_rules/${ruleId}`
        );
    }

    /**
     * Get automation rule errors for a tab.
     */
    getAutomationRuleErrors(groupId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/automation_rules/errors`
        );
    }

    /**
     * Get a datasource by tag.
     */
    getDatasourceTag(groupId: string, datasourceTag: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/datasource_tag/${datasourceTag}`
        );
    }

    /**
     * GET /tracking/search — term search across records/contacts.
     */
    searchTracking(term: string, parameters?: Record<string, string>) {
        const params = { term, ...parameters };
        return XenquAPI.Base.makeGet("/tracking/search", params);
    }
    ...
    searchLibraries(groupId: string, query: string) {
        return XenquAPI.Base.makePost(
            `/tracking/groups/${groupId}/libraries`,
    ...
    getContactLog(groupId: string, contactId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/contact_log/${contactId}`
        );
    }
}
