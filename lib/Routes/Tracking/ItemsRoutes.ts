...
export default class ItemsRoutes {
    ...
    getAssignedItemList(
        ...
        return XenquAPI.Base.makeGet(
            `/tracking/user/items?queued=${queued}&sortby=${sort}&offset=${offset}&count=${count}`
        );
    }
    ...
    getTrackingItem(trackingId: string, groupId: string, queueId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/queues/${queueId}/items/${trackingId}`
        );
    }

    /**
     * Get tracking item payload (form/actors/defaults scope).
     */
    getTrackingItemData(
        trackingId: string,
        groupId: string,
        queueId: string,
        scope: "defaults" | "form" | "actors" = "form"
    ) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/queues/${queueId}/items/${trackingId}/data`,
            { scope }
        );
    }
    ...
    updateTrackingItem(
        ...
        ignoreTimezones: boolean = false
    ) {
        return XenquAPI.Base.makePut(
            `/tracking/groups/${groupId}/queues/${queueId}/items/${trackingId}`,
            JSON.stringify(trackingItem.toJson(false))
        );
    }
    ...
    addMessage(trackingId: string, message: string) {
        return XenquAPI.Base.makePost(
            `/tracking/items/${trackingId}/logs`,
            JSON.stringify({ event_data: { message_text: message } })
        );
    }
    ...
    setCallback(
        ...
        return XenquAPI.Base.makePut(
            `/tracking/groups/${groupId}/queues/${queueId}/items/${trackingId}/callback`,
            JSON.stringify({ callback_url: url })
        );
    }
    ...
    removeCallback(
        ...
        return XenquAPI.Base.makeDelete(
            `/tracking/groups/${groupId}/queues/${queueId}/items/${trackingId}/callback`,
            JSON.stringify({ callback_url: url })
        );
    }
    ...
    getActivityStream(groupId: string, count: number = 5, offset: number = 0) {
        return XenquAPI.Base.makeGet(
            `/tracking/groups/${groupId}/items/activity?count=${count}&offset=${offset}`
        );
    }
    ...
    getDownload(attachmentId: string, fileId: string) {
        return XenquAPI.Base.makeGet(
            `/tracking/attachments/${attachmentId}/files/${fileId}`
        );
    }
    ...
    addAttachment(
        ...
        data: {
            filename: string;
            _temp_handle_id: string;
            tracking_id: number;
            content_type: string;
        }
    ) {
        return XenquAPI.Base.makePost(
            `/tracking/attachments/${attachmentId}/files`,
            JSON.stringify(data)
        );
    }
    ...
    updateAttachment(
        attachmentId: string,
        data: { tracking_id: number; filename: string; _temp_handle_id: string }
    ) {
        return XenquAPI.Base.makePut(
            `/tracking/attachments/${attachmentId}/files`,
            JSON.stringify(data)
    ...
    runRulesOnAttachment(itemId: string, ruleRoute: string[]) {
        return XenquAPI.Base.makeGet(
            `/tracking/attachments/${itemId}/run_rules/${ruleRoute.join("/")}`
        );
    }
}
